...
@autotvm.template("dynconv/group_conv_dense")
def schedule_group_conv_dense(Cout, Cin, H, W, KH, KW, group):
    # dataflow
    input = te.placeholder((1, Cin, H, W), dtype="float32", name="input")
    weight = te.placeholder((Cout, Cin//group, KH, KW), dtype="float32", name="weight")

    def find_group(f): return (f // (Cout // group)) * (Cin // group)

    rc = te.reduce_axis((0, Cin//group), "rc")
    ry = te.reduce_axis((0, KH), "ry")
    rx = te.reduce_axis((0, KW), "rx")
    output = te.compute(
        (1, Cout, H-KH+1, W-KW+1),
        lambda n, f, y, x: te.sum(
            input[n, find_group(f)+rc, y + ry, x + rx] * weight[f, rc, ry, rx],
            axis=[rc, ry, rx]
        ),
        name="output"
    )
...
class GroupConvDenseScheduler(ConvDenseScheduler):
    def __init__(self, channel_out, channel_in, width, kernel, group, save_path):
        # group = min(group, channel_in)
        # group = min(group, channel_out)
        # in case group is not expected
        assert channel_in % group == 0
        assert channel_out % group == 0
        super().__init__(channel_out, channel_in, width, kernel, save_path)
        self.group = group

    def __repr__(self) -> str:
        return "GroupConvDenseScheduler"

    @property
    def _task_name(self):
        return "dynconv/group_conv_dense"

    @property
    def _task_args(self):
        return [self.channel_out, self.channel_in, self.width, self.width, self.kernel, self.kernel, self.group]

    @property
    def _schedule(self):
        return schedule_group_conv_dense(*self._task_args)
    ...
    def _generate_sample(self):
        ...
        group = self.group
        input = np.random.randn(1, Cin, H, W).astype("float32")
        weight = np.random.randn(Cout, Cin//group, KH, KW).astype("float32")
        return [input, weight]

    def _convert_sample(self, sample):
        return super()._convert_sample(sample)

    def _run_tvm(self, inputs):
        return super()._run_tvm(inputs)
    ...
    def _run_pytorch(self, inputs):
        ...
        input = torch.tensor(input, dtype=torch.float32, device="cuda")
        weight = torch.tensor(weight, dtype=torch.float32, device="cuda")
        output = F.conv2d(input, weight, groups=self.group)
        output = output.cpu().numpy()
        return output